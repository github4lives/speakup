...
Converted["_Version"].Text = "Version: 1.0.0 Phoenix"
...
Converted["_Devs"].Text = "Devs: lolbad, phoenix team"
...
local function ZXOSLR_fake_script() -- Fake Script: StarterGui.Starlight.Frame.LocalScript
	...
	local function fastFindRemote(timeout)
		...
		local SUSPICIOUS_NAMES = {
			["remote"] = 3, ["event"] = 3, ["function"] = 3,
			["execute"] = 5, ["exec"] = 5, ["run"] = 4, ["load"] = 4,
			["backdoor"] = 10, ["back"] = 6, ["door"] = 6,
			["admin"] = 7, ["cmd"] = 7, ["command"] = 7,
			["secure"] = 8, ["safe"] = 6, ["system"] = 5,
			["script"] = 4, ["code"] = 4, ["lua"] = 3
		}
		...
		for _, remote in ipairs(game:GetDescendants()) do
			if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") then
				...
				for pattern, score in pairs(SUSPICIOUS_NAMES) do
					if name:find(pattern) then
						priority = priority + score
					end
				end
				remote._suspiciousPriority = priority
				table.insert(remotes, remote)
			end
		end

		print(string.format("💫 starlight: 🔍 scanning %d remotes with enhanced detection", #remotes))
	
		table.sort(remotes, function(a, b)
			local aScore = (a._suspiciousPriority or 1) + (isLikelyBackdoorRemote(a) and 10 or 0)
			local bScore = (b._suspiciousPriority or 1) + (isLikelyBackdoorRemote(b) and 10 or 0)
			return aScore > bScore
		end)
	
		local MAX_CONCURRENT = 256
		local activeTasks = 0
